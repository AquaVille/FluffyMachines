...
public class SuperheatedFurnace extends NonHopperableBlock {
        ...
        new BlockMenuPreset(getId(), "&cFoundry") {
            ...
            public void newInstance(@Nonnull BlockMenu menu, @Nonnull Block b) {
                if (BlockStorage.getLocationInfo(b.getLocation(), "stored") == null) {
                    ...
                    menu.replaceExistingItem(4, new SlimefunItemStack("DUST_AVAILABLE",Material.GUNPOWDER, "&6Dust Available: &e0", "&a> &eLeft Click &ahere to retrieve 1", "&a> &eLeft Click &ahere to retrieve 64").item());
                    menu.replaceExistingItem(7, new SlimefunItemStack("INGOTS_AVAILABLE",Material.IRON_INGOT, "&6Ingots Available: &e0", "&a> &eRight Click &ahere to retrieve 1", "&a> &eLeft Click &ahere to retrieve 64").item());
                    menu.replaceExistingItem(1, new SlimefunItemStack("DUST_MELTED",Material.CHEST, "&6Melted Dust: &e0 &7(0%)", "&bType: None",  "&7Stacks: 0").item());
                    ...
                    if (stored > 0) {
                        ...
                        if (stored > OVERFLOW_AMOUNT) {
                            ...
                            while (toRemove >= stackSize) {
                                ...
                                b.getWorld().dropItemNaturally(b.getLocation(), new SlimefunItemStack("DUST_ITEM",dust).asQuantity(stackSize));

                                toRemove = toRemove - stackSize;
                            }

                            if (toRemove > 0) {
                                b.getWorld().dropItemNaturally(b.getLocation(), new SlimefunItemStack("DUST_ITEM",dust).asQuantity(toRemove));
                            }
                            ...
                            if (stored > 0) {
                                b.getWorld().dropItemNaturally(b.getLocation(), new SlimefunItemStack("DUST_ITEM",dust).asQuantity(stored));
                            }
    ...
    protected void constructMenu(BlockMenuPreset preset) {
        for (int i : dustOutputBorder) {
            preset.addItem(i, new SlimefunItemStack("ITEM_BORDER",new ItemStack(Material.ORANGE_STAINED_GLASS_PANE), " ").item(), (p, slot, item, action) -> false);
        }

        for (int i : inputBorder) {
            preset.addItem(i, new SlimefunItemStack("ITEM_BORDER",new ItemStack(Material.CYAN_STAINED_GLASS_PANE), " ").item(), (p, slot, item, action) -> false);
        }

        for (int i : ingotOutputBorder) {
            preset.addItem(i, new SlimefunItemStack("ITEM_BORDER",new ItemStack(Material.RED_STAINED_GLASS_PANE), " ").item(), (p, slot, item, action) -> false);
        }
    ...
    private void updateIndicator(Block b) {
        ...
        if (stored.equals("0")) {
            ...
            inv.replaceExistingItem(INPUT_INDICATOR, new SlimefunItemStack("INPUT_INDICATOR",new ItemStack(Material.CHEST), "&6Melted Dust: &e0 &7(0%)", "&bType: None",  "&7Stacks: 0").item());
        } else {
            inv.replaceExistingItem(INPUT_INDICATOR, new SlimefunItemStack("INPUT_INDICATOR",new ItemStack(Material.CHEST), "&6Melted Dust: &e" + stored + " &7(" + Double.parseDouble(stored) / MAX_STORAGE * 100 + "%)", "&bType: " + type, "&7Stacks: " + Double.parseDouble(stored) / 64).item());

        }
        inv.replaceExistingItem(DUST_INDICATOR, new SlimefunItemStack("DUST_INDICATOR",new ItemStack(Material.GUNPOWDER), "&6Dust Available: &e" + stored, "&a> &eLeft Click &ahere to retrieve 1", "&a> &eRight Click &ahere to retrieve 64").item());
        inv.replaceExistingItem(INGOT_INDICATOR, new SlimefunItemStack("INPUT_INDICATOR",new ItemStack(Material.IRON_INGOT), "&6Ingots Available: &e" + stored, "&a> &eLeft Click &ahere to retrieve 1", "&a> &eRight Click &ahere to retrieve 64").item());
    ...
    private void retrieveDust(BlockMenu menu, Block b, boolean isRightClicked) {
        ...
        if (stored > 0 && (menu.getItemInSlot(DUST_OUTPUT_SLOT) == null
            ...
            ItemStack dustItem = new SlimefunItemStack("DUST_ITEM",SlimefunItem.getById(type + "_DUST").getItem().clone()).asQuantity(amount);
    ...
    private void retrieveIngot(BlockMenu menu, Block b, boolean isRightClicked) {
        ...
        if (stored > 0 && (menu.getItemInSlot(INGOT_OUTPUT_SLOT) == null
            ...
            ItemStack ingotItem;
            if (type.equals("GOLD")) {
                ingotItem = new SlimefunItemStack("INGOT_ITEM",SlimefunItems.GOLD_4K.getItem().getItem().clone()).asQuantity(amount);
            } else if (type.equals("IRON")) {
                ingotItem = new ItemStack(Material.IRON_INGOT, amount);
            } else {
                ingotItem = new SlimefunItemStack("INGOT_ITEM",SlimefunItem.getById(type + "_INGOT").getItem().clone()).asQuantity(amount);
            }
